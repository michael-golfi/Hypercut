...
import hypercut.bucket.AbundanceBucket
import hypercut.bucket.CountingSeqBucket
...
object DistinctByteBucket {
  ...
  class Unpacker(val itemSize: Int) extends hypercut.bucketdb.Unpacker[Array[Byte], DistinctByteBucket] {
...
object PackedSeqBucket {
  ...
  class Unpacker(dbLocation: String, minAbundance: Option[Abundance], buckets: Int)
    extends hypercut.bucketdb.Unpacker[String, PackedSeqBucket] {
