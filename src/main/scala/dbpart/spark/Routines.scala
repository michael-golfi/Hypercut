...
class Routines(spark: SparkSession) {
  ...
  def splitReads(reads: Dataset[String],  ext: MarkerSetExtractor): Dataset[ProcessedRead] = {
    reads.map(r => {
      ...
      ext.splitRead(r, buckets).map(x => (x._1.compact.data, x._2))
    })
  }
   ...
   */
  def edgesFromSplit(reads: Dataset[ProcessedRead]) = reads.flatMap(s => MarkerSetExtractor.collectTransitions(s.map(_._1)))
